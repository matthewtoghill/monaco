...
public static class MediatorExtensions
{
	extension(ISender sender)
	{
		...
		/// <param name="query"></param>
		/// <returns></returns>
		public async Task<Results<FileStreamHttpResult, NotFound>> ExecuteFileDownloadAsync<TResult>(QueryBase<TResult?> query) where TResult : FileDownloadDto
		{
			var item = await sender.Send(query);
		...
		/// <param name="query"></param>
		/// <returns></returns>
		public async Task<Results<FileStreamHttpResult, NotFound>> ExecuteFileDownloadAsync<TResult>(QueryByIdBase<TResult?> query) where TResult : FileDownloadDto
		{
			var item = await sender.Send(query);
			return GetFileDownload(item);
		}
		...
		/// <returns></returns>
		public async Task<Results<Created<TResult>, NotFound, ValidationProblem>> ExecuteCommandAsync<TResult>(CommandBase<TResult> command,
																											   string resultUri,
		...
		/// <param name="command"></param>
		/// <returns></returns>
		public async Task<Results<NoContent, NotFound, ValidationProblem>> ExecuteCommandEditAsync(CommandBase command)
		{
			var result = await sender.Send(command);
			return result switch
				   {
		...
		/// <param name="command"></param>
		/// <returns></returns>
		public async Task<Results<Ok, NotFound, ValidationProblem>> ExecuteCommandDeleteAsync(CommandBase command)
		{
			var result = await sender.Send(command);
			return result switch
				   {
